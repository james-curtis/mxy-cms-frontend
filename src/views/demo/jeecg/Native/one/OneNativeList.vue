<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" pre-icon="ant-design:plus" @click="handleAdd">
        新增
      </a-button>
...
const columns = ref<any>([
  ...
  },
  {
    ...
    dataIndex: 'xiala',
    customRender: ({ text }) =>
      text ? filterMultiDictText(dictOptions.value['xiala'], text) : '',
  },
  {
    title: '字典单选',
    align: 'center',
    dataIndex: 'danxuan',
    customRender: ({ text }) =>
      text ? filterMultiDictText(dictOptions.value['danxuan'], text) : '',
  },
  {
    title: '字典多选',
    align: 'center',
    dataIndex: 'duoxuan',
    customRender: ({ text }) =>
      text ? filterMultiDictText(dictOptions.value['duoxuan'], text) : '',
  },
  {
    title: '开关',
    align: 'center',
    dataIndex: 'kaiguan',
    customRender: ({ text }) =>
      text ? filterMultiDictText(dictOptions.value['kaiguan'], text) : '',
  },
  {
    title: '日期',
    align: 'center',
...
function batchDel() {
  Modal.confirm({
    ...
    onOk: () => {
      defHttp
        .delete(
          { url: Api.delete, data: { ids: selectedRowKeys.value } },
...
function getImgView(text) {
  if (text && text.indexOf(',') > 0) {
    text = text.slice(0, Math.max(0, text.indexOf(',')));
  }
...
function handleDelete(id) {
  defHttp
    .delete({ url: Api.delete, data: { ids: id } }, { joinParamsToUrl: true })
    .then((res) => {
      handleSuccess();
    });
}
