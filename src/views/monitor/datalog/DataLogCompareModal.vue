...
<template>
  <div>
    <BasicModal
      ...
      @register="register"
    >
      <a-row v-if="dataVersionList" :gutter="6" style="margin-left: 2px">
        <span style="margin-top: 5px; margin-right: 3px; margin-left: 4px"
          >版本对比:</span
        >
        <a-select
      ...
      >
        <template #dataVersionTitle1="{ record }">
          <Icon icon="icon-park-outline:grinning-face" /> 版本:{{
            dataVersion1Num
          }}
        </template>
        ...
        <template #avatarslot="{ record }">
          <div
            v-if="record.dataVersion1 != record.dataVersion2"
            class="anty-img-wrap"
          >
