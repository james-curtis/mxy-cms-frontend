<template>
  <div class="p-4">
    <BasicTable :data-source="dataSource" @register="registerTable">
      <template #tableTitle>
        <div slot="message">
      ...
      <template #responseStatus="{ record, text }">
        <a-tag v-if="text < 200" color="pink">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text < 201" color="green">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text < 399" color="cyan">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text < 403" color="orange">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text < 501" color="red">
          {{ text }}
        </a-tag>
        <span v-else>{{ text }}</span>
      </template>
      <template #requestMethod="{ record, text }">
        <a-tag v-if="text === 'GET'" color="#87d068">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text === 'POST'" color="#2db7f5">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text === 'PUT'" color="#ffba5a">
          {{ text }}
        </a-tag>
        <a-tag v-else-if="text === 'DELETE'" color="#f50">
          {{ text }}
        </a-tag>
  ...
  getActuatorList().then((res) => {
    ...
    for (const d of res.traces) {
      if (
        ...
        !d.request.uri.includes('httptrace')
      ) {
        filterData.push(d);
      }
    }
