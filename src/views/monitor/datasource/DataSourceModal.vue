      ...
      <template #pwd="{ model, field }">
        <a-row :gutter="8">
          ...
          <a-col :sm="9" :md="7" :lg="7" :xl="5">
            <a-button type="primary" style="width: 100%" @click="handleTest">
              测试
            </a-button>
          </a-col>
        </a-row>
...
const [
  registerForm,
  { getFieldsValue, resetFields, validateFields, setFieldsValue, validate },
] = useForm({
  labelWidth: 150,
  schemas: formSchema,
...
async function handleTest() {
  ...
  const setFields = {};
  keys.forEach(
    (key) => (setFields[key] = { value: fieldsValues[key], errors: null })
  );
