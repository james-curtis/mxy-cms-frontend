  ...
  <div>
    ...
    <BasicTable :row-selection="rowSelection" @register="registerTable">
      ...
      <template #tableTitle>
        ...
        <a-button
          type="primary"
          pre-icon="ant-design:hdd-outlined"
        ...
        />
        <a-button type="primary" pre-icon="ant-design:filter-outlined">
          高级查询?
        </a-button>
...
async function batchHandleDelete() {
  const hasAdmin = unref(selectedRows).filter(
...
function batchFrozen(status) {
  const hasAdmin = selectedRows.value.filter(
    (item) => item.username == 'admin'
  );
  if (unref(hasAdmin).length > 0) {
    createMessage.warning('管理员账号不允许此操作！');
  ...
  createConfirm({
    ...
    onOk: async () => {
      await frozenBatch(
        { ids: unref(selectedRowKeys).join(','), status },
        reload
      );
    },
  });
