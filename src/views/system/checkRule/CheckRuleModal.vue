<template>
  ...
  >
    ...
    <a-tabs v-model:activeKey="activeKey" animated>
      <a-tab-pane key="1" tab="局部规则" :force-render="true">
        <JVxeTable
          ref="vTable1"
          toolbar
          row-number
      ...
      <a-tab-pane key="2" tab="全局规则" :force-render="true">
        <JVxeTable
          ref="vTable2"
          toolbar
          row-number
  ...
  validate()
    ...
    .then((tableData2: []) => {
      ...
      const firstGlobal: any[] = [],
        afterGlobal: any[] = [];
      for (const v: any of globalValues) {
        v.digits = '*';
        if (v.priority === '1') {
          firstGlobal.push(v);
        } else {
      ...
      const subValues = concatValues.map((i) =>
        pick(i, 'digits', 'pattern', 'message')
      );
      // 生成 formData，用于传入后台
