 ...
 */
export const queryIdTree = (params?) =>
  defHttp.get({ url: Api.queryIdTree, params });

/**
 * 根据关键字搜索部门
 */
export const searchByKeywords = (params) =>
  defHttp.get({ url: Api.searchBy, params });

/**
 * 查询部门下的用户信息
 */
export const departUserList = (params) =>
  defHttp.get({ url: DepartUserApi.list, params });
...
export const linkDepartUserBatch = (departId: string, userIdList: string[]) =>
  defHttp.post({
    url: DepartUserApi.link,
    params: { depId: departId, userIdList },
  });
...
export const unlinkDepartUserBatch = (params, confirm = false) => {
  return new Promise((resolve, reject) => {
    const doDelete = () => {
      resolve(
        defHttp.delete(
          { url: DepartUserApi.unlink, params },
 ...
 * 查询部门角色信息
 */
export const departRoleList = (params) =>
  defHttp.get({ url: DepartRoleApi.list, params });
...
export const deleteBatchDepartRole = (params, confirm = false) => {
  return new Promise((resolve, reject) => {
    const doDelete = () => {
      resolve(
        defHttp.delete(
          { url: DepartRoleApi.deleteBatch, params },
          { joinParamsToUrl: true }
        )
      );
    };
 ...
 *  查询部门角色数据权限列表
 */
export const queryDepartRoleDataRule = (
  functionId,
  departId,
  roleId,
  params?
) => {
  const url = `${DepartRoleApi.dataRule}/${unref(functionId)}/${unref(
    departId
  )}/${unref(roleId)}`;
