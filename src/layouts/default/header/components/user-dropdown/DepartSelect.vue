<template>
  <BasicModal
    ...
    :title="currTitle"
    wrap-class-name="loginSelectModal"
  >
    <a-form
      ref="formRef"
      v-bind="layout"
...
import { computed, defineExpose, ref, unref, watch } from 'vue';
import { Avatar } from 'ant-design-vue';
import { BasicModal } from '/@/components/Modal';
import {
  getUserDeparts,
  selectDepart,
} from '/@/views/system/depart/depart.api';
...
async function loadDepartList() {
  ...
  const currentDepart = result.list.filter(
    (item) => item.orgCode == result.orgCode
  );
  departList.value = result.list;
  departSelected.value =
    currentDepart && currentDepart.length > 0 ? result.orgCode : '';
  currentDepartName.value =
    currentDepart && currentDepart.length > 0
      ? currentDepart[0].departName
      : '';
