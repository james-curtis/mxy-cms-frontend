<template>
  <div :class="prefixCls">
    <Popover
      ...
      title=""
      trigger="click"
      :overlay-class-name="`${prefixCls}__overlay`"
    >
      <Badge
        :count="count"
        :overflow-count="9"
      ...
      <template #content>
        ...
        <a-row class="bottom-buttons">
          <a-col :span="count === 0 ? 0 : 12">
            <a-button type="dashed" block @click="onEmptyNotify">
              清空消息
            </a-button>
          </a-col>
          <a-col :span="count === 0 ? 24 : 12">
            <a-button type="dashed" block @click="popoverVisible = false">
...
export default defineComponent({
  ...
  setup() {
    ...
    async function loadData() {
      try {
        const { anntMsgList, sysMsgList, anntMsgTotal, sysMsgTotal } =
          await listCementByUser({
            pageSize: 5,
          });
    ...
    function initWebSocket() {
      ...
      const url = `${glob.domainUrl
        ?.replace('https://', 'wss://')
        .replace('http://', 'ws://')}/websocket/${userId}`;
