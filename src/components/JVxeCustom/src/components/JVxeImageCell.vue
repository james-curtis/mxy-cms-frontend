<template>
  <div>
    <template
      v-for="(file, fileKey) of [innerFile || {}]"
      v-if="hasFile"
      :key="fileKey"
    >
      <div class="j-vxe-image-list">
        <template
          v-if="!file || !(file['url'] || file['path'] || file['message'])"
        >
          <a-tooltip
            :title="
              '请稍后: ' +
              JSON.stringify(file) +
              (file['url'] || file['path'] || file['message'])
            "
          >
            <LoadingOutlined />
          </a-tooltip>
        ...
        <a-tooltip
          v-else
          :title="file.message || '上传失败'"
          @click="handleClickShowImageError"
        >
    ...
    <div class="j-vxe-image-upload">
      ...
      >
        <a-button v-if="!hasFile" pre-icon="ant-design:upload">
          {{ originColumn.btnText || '上传图片' }}
        </a-button>
        <div
          v-if="hasFile && imgList.length < maxCount"
          class="j-vxe-plus"
          @click=""
        >
