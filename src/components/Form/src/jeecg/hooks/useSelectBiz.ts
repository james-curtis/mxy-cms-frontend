...
export function useSelectBiz(getList, props) {
  ...
  //接收已选择的值
  const selectValues = <object>(
    inject('selectValues', reactive({ value: [], change: false }))
  );
  // 是否正在加载回显
  ...
  async function onSelectChange(
    ...
    if (
      props.showSelected &&
      unref(checkedKeys).length !== unref(selectRow).length
    ) {
  ...
  function getSelectResult(success) {
    ...
    if (props.maxSelectCount && values.length > props.maxSelectCount) {
      $message.createMessage.warning(
        `最多只能选择${props.maxSelectCount}条数据`
      );
      return false;
    }
  ...
  function handleDeleteSelected(record) {
    checkedKeys.value = checkedKeys.value.splice(
      checkedKeys.value.indexOf(record['id']),
      1
    );
