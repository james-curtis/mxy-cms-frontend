...
watch(fileList, () => nextTick(() => addActionsListener()), {
  immediate: true,
});
...
function addActionsListener() {
  ...
  for (const uploadItem of uploadItems) {
    const hasActions = uploadItem.dataset.hasActions === 'true';
    if (!hasActions) {
...
function onAddActionsButton(event) {
  ...
  const actions = uploadItem.querySelectorAll('.ant-upload-list-item-actions');
  ...
  uploadItem.dataset.hasActions = 'true';
