  ...
  <div>
    ...
    >
      <BasicTree
        ...
        :check-strictly="getCheckStrictly"
        @select="onSelect"
        @check="onCheck"
      />
      <!--树操作部分-->
      <template #insertFooter>
        <a-dropdown placement="top">
          <template #overlay>
            <a-menu>
              <a-menu-item v-if="multiple" key="1" @click="checkALL(true)">
                全部勾选
              </a-menu-item>
              <a-menu-item v-if="multiple" key="2" @click="checkALL(false)">
                取消全选
              </a-menu-item>
              <a-menu-item key="3" @click="expandAll(true)">
                展开全部
              </a-menu-item>
              <a-menu-item key="4" @click="expandAll(false)">
                折叠全部
              </a-menu-item>
            </a-menu>
          </template>
...
export default defineComponent({
  ...
  setup(props, { emit, refs }) {
    ...
    function getQueryUrl() {
      ...
      return (params) =>
        queryFn(Object.assign({}, params, { primaryKey: props.rowKey }));
