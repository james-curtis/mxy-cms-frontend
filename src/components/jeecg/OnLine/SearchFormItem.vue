<template>
  ...
  >
    ...
    <template v-else>
      ...
      />
      <span class="group-query-strig" style="width: auto; padding: 0 4px"
        >~</span
      >
      <a-date-picker
        v-model:value="queryParam[item.field + '_end']"
  ...
  >
    ...
    >
      <template
        v-for="(obj, index) in dictOptions[getDictOptionKey(item)]"
        :key="index"
      >
        <a-select-option :value="obj.value">
          {{ obj.text }}
        </a-select-option>
      </template>
    </a-select>
  ...
  >
    ...
    <JTreeSelect
      ...
      :placeholder="'请选择' + item.label"
      :dict="item.dict"
      :pid-field="item.pidField"
      :pid-value="item.pidValue"
      :has-child-field="item.hasChildField"
      load-triggle-change
    />
  </a-form-item>
  ...
  <a-form-item
    v-else-if="
      item.view === CompTypeEnum.Checkbox ||
      item.view === CompTypeEnum.ListMulti
    "
    :label-col="labelCol"
    :label="item.label"
    :class="'jeecg-online-search'"
  ...
  <a-form-item v-else :label-col="labelCol" :class="'jeecg-online-search'">
    <template #label>
      <span :title="item.label" class="label-text">{{ item.label }}</span>
    </template>
    <template v-if="single_mode === item.mode">
