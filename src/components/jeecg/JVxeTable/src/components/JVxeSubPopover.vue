<template>
  <a-popover
    :visible="visible"
    :placement="placement"
    overlay-class-name="j-vxe-popover-overlay"
  ...
  >
    ...
    <div ref="divRef" class="j-vxe-popover-div" />
  </a-popover>
</template>
<script lang="ts">
  ...
  setup() {
    ...
    function open(event, level = 0) {
      ...
      if (parentElem && tr) {
        ...
        overlayStyle.width = `${Number.parseInt(
          `${clientWidth - clientWidth * 0.04}`
        )}px`;
        ...
        const num = ++level;
        console.warn(
          `【JVxeSubPopover】table or tr 获取失败，正在进行第 ${num}次重试`,
          {
            event,
            table: parentElem,
            tr,
