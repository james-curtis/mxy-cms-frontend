  ...
  setup(props: JVxeComponent.Props) {
    ...
    const selectOptions = computed(() => {
      ...
      if (linkage) {
        ...
        const res = getLinkageOptionsSibling(
          row.value,
          originColumn.value,
          config,
          true
    ...
    function handleChange(value) {
      ...
      if (linkage) {
        linkage.handleLinkageSelectChange(
          row.value,
          originColumn.value,
          linkage.config,
          value
    ...
    function handleSearchSelect(value) {
      ...
      if (allowSearch !== true && allowInput === true) {
        ...
        if (!flag && !!value) {
          ...
          options.push({ title: value, value, searchAdd: true });
        }
      }
    }
  ...
  enhanced: {
    ...
    translate: {
      ...
      async handler(value, ctx) {
        ...
        if (linkage) {
          const { getLinkageOptionsSibling, config } = linkage;
          const linkageOptions = getLinkageOptionsSibling(
            row.value,
            originColumn.value,
            config,
            true
          );
          options = isPromise(linkageOptions)
            ? await linkageOptions
            : linkageOptions;
