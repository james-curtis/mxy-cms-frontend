<template>
  <div
    :class="{ 'comment-active': commentActive }"
    style="border: 1px solid #eee; margin: 0; position: relative"
  ...
  >
    ...
    <div
      ...
      :class="{
        'no-content': noConent,
        'top-div': showHtml,
        'bottom-div': showHtml == false,
      }"
      @click="handleClickHtmlShower"
      v-html="commentHtml"
    />
    <div v-if="commentActive" class="comment-buttons">
      <div style="cursor: pointer">
        ...
        <span title="表情" style="display: inline-block">
          ...
          <div v-show="" style="position: relative" />
        </span>
      </div>
      <div v-if="commentActive">
        <a-button v-if="inner" style="margin-right: 10px" @click="noComment">
          取消
        </a-button>
        <a-button
          type="primary"
  ...
  />
  <a-modal
    v-model:visible="visibleEmoji"
    :footer="null"
    wrap-class-name="emoji-modal"
    :closable="false"
    :width="490"
  >
    <template #title>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function showEmoji(e) {
      ...
      let str = e.colons;
      if (str.indexOf('::') > 0) {
        str = str.slice(0, Math.max(0, str.indexOf(':') + 1));
      }
      myComment.value = temp + str;
