...
export default class signMd5Utils {
  ...
  static parseQueryString(url) {
    ...
    let lastpathVariable = url.slice(Math.max(0, url.lastIndexOf('/') + 1));
    if (lastpathVariable.includes(',')) {
      if (lastpathVariable.includes('?')) {
        lastpathVariable = lastpathVariable.slice(
          0,
          Math.max(0, lastpathVariable.indexOf('?'))
        );
      }
      //解决Sign 签名校验失败 #2728
  ...
  static urlEncode(param, key, encode) {
    ...
    if (t == 'string' || t == 'number' || t == 'boolean') {
      ...
      for (const i in param) {
        const k =
          key == null ? i : key + (Array.isArray(param) ? `[${i}]` : `.${i}`);
